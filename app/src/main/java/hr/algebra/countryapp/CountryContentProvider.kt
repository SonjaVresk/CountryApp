...
private const val AUTHORITY = "hr.algebra.countryapp.api.provider"
...
class CountryContentProvider : ContentProvider() {
    ...
    override fun delete(uri: Uri, selection: String?, selectionArgs: Array<String>?): Int {
        val tableName = when (URI_MATCHER.match(uri)) {
            ITEMS -> TABLE_NAME_COUNTRY
            ITEM_ID -> TABLE_NAME_COUNTRY
            ...
            return when (URI_MATCHER.match(uri)) {
                ...
                ITEM_ID -> {
                    uri.lastPathSegment?.let { id ->
                        repository.update(tableName, values, "${Country::_id.name}=?", arrayOf(id))
                    }
                        ?: 0
                }
