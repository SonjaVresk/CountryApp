...
private const val AUTHORITY = "hr.algebra.countryapp.api.provider.monument"
private const val PATH = "countries"
val MONUMENT_PROVIDER_CONTENT_URI: Uri = Uri.parse("content://$AUTHORITY/$PATH")
...
class MonumentContentProvider : ContentProvider() {

    private lateinit var repository: Repository

    override fun delete(uri: Uri, selection: String?, selectionArgs: Array<String>?): Int {
        val tableName = when (URI_MATCHER.match(uri)) {
            ITEMS -> TABLE_NAME_MONUMENT
            ITEM_ID -> TABLE_NAME_MONUMENT
    ...
    override fun insert(uri: Uri, values: ContentValues?): Uri? {
        ...
        return ContentUris.withAppendedId(MONUMENT_PROVIDER_CONTENT_URI, id)
    }
        ...
        return when (URI_MATCHER.match(uri)) {
            ...
            ITEM_ID -> {
                uri.lastPathSegment?.let { id ->
                    repository.update(tableName, values, "${Monument::_id.name}=?", arrayOf(id))
                }
                    ?: 0
            }
